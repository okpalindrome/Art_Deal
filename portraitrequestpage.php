...
<br>
    ...
    <br>
  <h2 style="font-size:35px;color:grey" align="center" >To draw your portrait I need some data.</h2>
  <div class="clear:both"></div>
<br>
  <div class="clear:both"></div>
...
<form method="post" enctype="multipart/form-data">
								...
								<div id="gridd" style="width: 30%">
											...
											<b><input type="text" name="p_portrait" value="<?php echo $varrpp ?>" style="border:none;" readonly ></b>
								</div>
								<div id="gridd" style="width: 30%">
											<span style="margin-left: 2%">Type of portrait<label>*</label></span>
											<select name="ptype">
								...
								<div id="gridd" style="width: 30%">
											<span>Upload your image<label>*</label></span>
											<input type="file" value="Upload images" name="userpimg" required accept="image/*"> 
										</div>
								</div>
								<br clear="both">
								<br>
							<input type="submit" id="s123" value="Request" name="pbtn" style="float: right;" />	
			</form>
</div>
<br clear="both">
<?php
if (isset($_POST['pbtn'])) {
	...
	$filename1=$_FILES['userpimg']['name'];
	$tmpname1=$_FILES['userpimg']['tmp_name'];
	$target1='userportraitimgs/';
    
	$prq=mysql_query("INSERT INTO `pwt`(`artist_id`, `user_id`,`portrait`,`wallpaint`,`teach`,`type`, `user_img`, `price`,`status`) 
		VALUES ('$varr','$varuid','1','','','$vartype','$filename1','$varrpp','wait')");
	if ($prq) {
		move_uploaded_file($tmpname1, $target1.$filename1);
		?>
		<script type="text/javascript">
			alert('Successfully uploaded and requested to the artist');
		</script><?php
	}
}
?>
